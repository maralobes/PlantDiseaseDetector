...
def preprocess_image(image: Image.Image):  
    image = image.resize((128, 128))  # Ajustar al tamaño que espera el modelo  
    image_array = np.array(image) / 255.0  # Normalizar los valores de los píxeles  
    image_array = np.expand_dims(image_array, axis=0)  # Añadir una dimensión para la predicción  
    return image_array  
...
async def analyze_image(file: UploadFile = File(...)):  
    try:  
        ...
        # Obtener la descripción y tratamiento de disease_info  
